...
import { FilterRule, XXXFiltering } from "./filteringParams";
import { XXSorting } from "./sortingParams";



export const getOrder = (sort: XXSorting) => sort ? { [sort.property]: sort.direction } : {};

export const getWhere = (filter: XXXFiltering) => {
    if (!filter) return {};
